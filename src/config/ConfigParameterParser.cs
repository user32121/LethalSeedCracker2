...
namespace LethalSeedCracker2.src.config
{
    ...
    {
        internal override void Process(Config config, Action<Predicate<CrackingResult>> filters, TextReader stream)
        {
            apply(config);
        }
    }
    ...
    {
        internal override void Process(Config config, Action<Predicate<CrackingResult>> filterConsumer, TextReader stream, List<T0> arg0s)
        {
            apply(config, arg0s);
        }
    }
    internal class ConfigMarkerParser(string cmd) : ConfigCommandParser(cmd)
    {
